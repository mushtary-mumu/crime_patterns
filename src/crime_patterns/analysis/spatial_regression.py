...
def prepare_data_for_spatial_regression(crime_data, explanatory_data, population, crime_col_name, population_col_name, ID_col_name, standardize=True):

    db = crime_data[[ID_col_name, crime_col_name]].merge(explanatory_data.drop("geometry", axis=1), on=ID_col_name)
...
def perform_spatial_regression(db, y_var_name, x_var_names, method="OLS"):
    ...
    if method == "OLS":
        model = OLS(y = y, x = x, w = W, 
            name_y=y_var_name, name_x = x_name, name_w="W", name_ds='Burglary_VS_IMDScores', 
            white_test=True, spat_diag=True, moran=True)

    elif method == "ML_Lag":
        model = ML_Lag(y = y, x = x, w = W, 
          name_y=y_var_name, name_x = x_name, name_w="W", name_ds='Burglary_VS_IMDScores')

    elif method == "ML_Error":
        model = ML_Error(y = y, x = x, w = W, 
          name_y=y_var_name, name_x = x_name, name_w="W", name_ds='Burglary_VS_IMDScores')

    else:
        raise ValueError("Invalid method. Valid options are: 'OLS', 'ML_Lag', 'ML_Error'.")
...
def get_spatial_diagnostics(model):
    ...
    lm_error = pd.Series({"Value": model.lm_error[0],
                          "p-value": model.lm_error[1]}, 
                          name="Lagrange Multiplier (error)")

    lm_lag = pd.Series({"Value": model.lm_lag[0],
     "p-value": model.lm_lag[1]}, name="Lagrange Multiplier (lag)")

    
    rlm_error = pd.Series({"Value": model.rlm_error[0],
     "p-value": model.rlm_error[1]}, name="Robust LM (error)")

    rlm_lag = pd.Series({"Value": model.rlm_lag[0],
                        "p-value": model.rlm_lag[1]}, name="Robust LM (lag)")

    morans_i = pd.Series({"Value":model.moran_res[0],
                        "p-value": model.moran_res[2]}, name="Moran's I")
