...
def test_evaluate_hotspots(mock_crime_points, mock_crime_polygons):
    ...
    ds = evaluate_hotspots( longitudes = mock_crime_points["points"].x, 
                            latitudes = mock_crime_points["points"].y, 
                            region = mock_crime_polygons, 
                            crs="EPSG:4326")
    
    assert np.isclose(float(ds.densities.max()), 11.90496, atol=pytest.DESIRED_PRECISION)
    assert np.isclose(float(ds.densities.min()), 1.00051, atol=pytest.DESIRED_PRECISION)
    assert np.isclose(float(ds.densities.mean()), 4.53039, atol=pytest.DESIRED_PRECISION)

#%%
def test_cluster_crime_incidents_dbscan(mock_crime_points):
    
    cluster_labels = cluster_crime_incidents_dbscan(longitudes = mock_crime_points["points"].x, 
                                                    latitudes = mock_crime_points["points"].y, 
                                                    epsilon=6, 
                                                    min_samples=10)
