...
def test_calculate_morans_I(mock_crime_polygons, mock_weights_matrix):
    ...
    moran = spatial_regression.calculate_morans_I(mock_crime_polygons, "crime_count", mock_weights_matrix)

    assert np.isclose(moran.I, -0.125, atol=pytest.DESIRED_PRECISION)


#%%
@pytest.mark.parametrize("method, expected_betas",
                        [
                        ("OLS", 
                        np.array([[ 0.04301335],
                                [ 0.09338169],
...
def test_perform_spatial_regression(mock_crime_polygons, method, expected_betas):
    ...
    if method in ["OLS", "ML_Lag", "ML_Error"]:
        model = spatial_regression.perform_spatial_regression(db=mock_crime_polygons, 
        ...
        with expected_betas:
            spatial_regression.perform_spatial_regression(db=mock_crime_polygons, 
                                                          y_var_name="crime_rate", 
                                                          x_var_names=["EmpScore", "IncScore", "BHSScore"], 
                                                          method=method)
